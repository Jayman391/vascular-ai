...
def store_subquestions(main_question, subquestions):
    ...
    for q in subquestions:
        c.execute("INSERT INTO subquestions (main_question, subquestion) VALUES (?, ?)", (main_question, q))
    conn.commit()
    conn.close()
...
def get_subquestions(main_question):
    conn = sqlite3.connect("questions.db")
    c = conn.cursor()
    c.execute("SELECT subquestion FROM subquestions WHERE main_question = ?", (main_question,))
    rows = c.fetchall()
    conn.close()
...
def main():
    ...
    if st.button("Submit"):
        with st.spinner("Processing..."):
            ...
            final_answer = final_chain(main_question, sub_context, citations_context)
